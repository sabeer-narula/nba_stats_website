...
const HighestPaidPlayers: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://127.0.0.1:5000/api/highest-paid-players');
        setHighestPaidPlayers(response.data);
      } catch (error) {
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6">
      <h2 className="text-2xl font-semibold mb-4">Highest Paid Players</h2>
...
export default HighestPaidPlayers;